document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".subcategory-link").forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      const targetElement = document.querySelector(targetId); // Target the specific command div
      if (targetElement) {
        const parentSection = targetElement.closest(".section"); // Find its parent section
        if (parentSection) {
          parentSection.classList.add("active"); // Make the parent section visible
        }

        const offset = 20;
        const targetPosition = targetElement.offsetTop - offset; // Scroll to the specific command
